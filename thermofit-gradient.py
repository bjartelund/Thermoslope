...
def process(datafiles):
    ...
    global startingconcentrations
    startingconcentrations= [topconcentration/dilution**x for x in range(0,positions)]

    mergeddataframes=pd.concat([processcsv(datafile) for datafile in datafiles]) #Collect all processed datasets in a single dataframe
